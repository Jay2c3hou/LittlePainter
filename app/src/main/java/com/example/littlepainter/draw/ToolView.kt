...
class ToolView(context: Context, attrs: AttributeSet?) : ViewGroup(context, attrs) {
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.ToolView).apply {
            mIcon = getDrawable(R.styleable.ToolView_icon) ?: resources.getDrawable(
                R.drawable.logo2,
                null
            )
            mBgIcon = getDrawable(R.styleable.ToolView_bg_icon)
                ?: resources.getDrawable(R.drawable.round_bg, null)
            ...
            // ToolType.values()会返回 ToolType 枚举类中定义的所有枚举值的数组
